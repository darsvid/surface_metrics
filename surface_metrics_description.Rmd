...
For illustration I am using two datasets from the ./data folder.

```{r, echo = FALSE, eval = TRUE, message = FALSE, fig.width = 5, fig.height = 5, fig.fullwidth = TRUE, fig.cap = "Digital elevation model"}
par(mar = c(.2, .2, .2, .2))
library(raster)
dem <- raster("./data/local_mean_32636.tif")
plot(dem, col = terrain.colors(255), axes = F, box = F)
